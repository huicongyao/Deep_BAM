...
//            at::Tensor signal_tensor = torch::from_blob(signal.data(), signal.size(), torch::kFloat32);
//            read->sample_data = signal_tensor.detach().clone();
            read->sample_data = torch::tensor(signal).to(torch::kFloat32);

//            read->sample_data = torch::from_blob(signal.data(), signal.size(), torch::kFloat32);
//            read->raw_signal.resize(signal.size(), 1);
//            Eigen::Map<const Eigen::MatrixXf> mapSignal(signal.data(), signal.size(), 1);
//            read->raw_signal = Eigen::MatrixXf(mapSignal);
...
//            if (read->read_id == "002409cc-11f6-4e6a-baed-d408cdaa98b6") {
//                std::cout << read->sample_data.data_ptr<float>() << std::endl;
//            }
...
//    std::cout << id_to_read[id]->sample_data.data_ptr<float>() << std::endl;
    return id_to_read[id]->sample_data.clone();
}

//Eigen::MatrixXf Yao::Pod5Data::get_raw_signal_by_read_id_eigen(std::string id) {
//    return id_to_read[id]->raw_signal;
//}
