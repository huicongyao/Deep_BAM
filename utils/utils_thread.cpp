...
//    int32_t SEQ_LEN = 4;
    ...
    auto convert_num2str = [&](int32_t i) -> std::string {
        /*
         * a lambda function to convert numbers to a fix size string
         * */
        ss.str("");
        ss.str("");
    ...
    for (auto &sam_ptr: inputs) {
        ...
        auto sig_len_v = Yao::group_signal_by_movetable(signal_group,
                                                      trimed_sig,
                                                      sam_ptr->movetable,
                                                      sam_ptr->stride,
                                                      SIG_LEN,
        ...
        at::Tensor sig_len_t = torch::from_blob(sig_len_v.data(), sig_len_v.size(), torch::kInt32);
        ...
        at::Tensor ref_signal_grp = torch::zeros({(int64_t)ref_seq.length(), SIG_LEN}, torch::kFloat32);
        ...
        for (const auto &off_loc: refloc_of_methysite) {
            if (num_bases <= off_loc && (off_loc + num_bases) < (int32_t) ref_seq.length()) {
                ...
                float label ; // label , 1 for methylated and 0 for un-methylated
...
//                    continue;
                ...
                at::Tensor signal_stds_t = torch::std(k_signals_rect, 1, true, true);

                at::Tensor signal_median_t =  std::get<0>(torch::median(k_signals_rect, 1, true));
...
//                torch::nan
    ...
    cv.notify_all();
    size_t len = kmer_size * (1 + SEQ_LEN + SIG_LEN) + 1;
...
void Yao::get_hc_features(Yao::Pod5Data p5,
    ...
    inputs.clear();
    size_t feature_len = kmer_size * (SEQ_LEN + SIG_LEN + 1) + 1;
...
//    int32_t SEQ_LEN = 4;
    ...
    at::Tensor signal_batch = torch::empty({batch_size, 1, kmer_size, SEQ_LEN + SIG_LEN});
    for (auto &sam_ptr: inputs) {
        auto coverage = Yao::compute_coverage(sam_ptr->cigar_pair, sam_ptr->query_alignment_length);
        auto identity = Yao::compute_pct_identity(sam_ptr->cigar_pair, sam_ptr->NM);
        if (sam_ptr->mapping_quality < mapq_thresh_hold || coverage < coverage_thresh_hold ||
...
//            spdlog::info("found splited read-{}", sam_ptr->query_name);
        ...
        for (const auto &off_loc: refloc_of_methysite) {
            if (num_bases <= off_loc && (off_loc + num_bases) < (int32_t) ref_seq.length()) {
                ...
                // call mods 先把左右两mer含c的去掉
//                int count = std::count(k_mer.begin(), k_mer.end(), 'C');
//                if (count > 1) continue;
...
//                                                   signal_median_t,
                                                   ...
                                                   k_seq_qual,
                                                   k_signals_rect}, 1)
                        .reshape({1, (long) kmer_size, SEQ_LEN + SIG_LEN});
...
//                    auto p5 = Yao::Pod5Data(p5_file);
////                    p5.get_raw_signal_by_read_id("002409cc-11f6-4e6a-baed-d408cdaa98b6");
//                    return p5;
//                };
...
//                    auto raw_signal = p5.get_raw_signal_by_read_id("002409cc-11f6-4e6a-baed-d408cdaa98b6");

//                    std::cout << raw_signal << std::endl;
...
Yao::Model_Inference(torch::jit::Module &module,
    ...
    while (true) {
        ...
        kmer = kmer.to(torch::kInt32);
        signal = signal.to(torch::kFloat16);
...
void Yao::count_modification_thread(std::queue<std::vector<std::string>> &site_key_Queue,
                                    std::queue<std::vector<std::string>> &site_info_Queue,
                                    std::queue<std::vector<int64_t>> &pred_Queue,
                                    std::queue<std::vector<float>> &p_rate_Queue,
                                    fs::path &write_file,
    ...
    while (true) {
        ...
        std::string uuid_pre;
        for (size_t j = 0; j < site_info.size(); j++) {
//            if (site_info[j].substr(0, 36) == "4bb0c827-0206-4a28-993d-5425f83a5bad") {
//                spdlog::info("found it");
//            }

            if (site_info[j].substr(0, 36) != uuid_pre ) {
                uuid_pre = site_info[j].substr(0, 36);
            }
            else {
                site_info[j].replace(0, 36,"*");
            }
        }
