...
at::Tensor Yao::interpolation(at::Tensor input, int output_size) {
//    input = input.view({1, 1, -1});
    namespace F = torch::nn::functional;
    at::Tensor interpolated = F::interpolate(input.view({1, 1, -1}),
                                             F::InterpolateFuncOptions().size(std::vector<int64_t>({output_size})).mode(torch::kLinear).align_corners(true)).view(output_size);
...
std::vector<int32_t> Yao::group_signal_by_movetable(at::Tensor &signal_group,
    ...
    for (size_t move_idx = 0; move_idx < movetable.size() - 1; move_idx++) {
        ...
        if (resample == true) {
            signal_group[move_idx] = Yao::interpolation(trimed_signal.index({Slice(start, end )}), signal_len);
        }
        else {
            ...
            if (vec_len[move_idx] < signal_len) {
                ...
                if (end > trimed_signal.size(0)) {
                    end = trimed_signal.size(0);
                    start = end - signal_len;
                }
            }
...
//        std::string key = line_s[0] + "#" + line_s[3] + "#" + line_s[4];
        std::string key = line_s[0] + "#" + std::to_string(std::stoi(line_s[1]) - 1);
        hc_sites.insert(key);
    }
